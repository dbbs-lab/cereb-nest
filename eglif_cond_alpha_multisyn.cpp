...
void eglif_cond_alpha_multisyn::update(nest::Time const &origin,
  ...
  if ( S_.y_[State_::I_input] < P_.ith){
        ...
        S_.y_[State_::I_adap] = (S_.y_[State_::I_input] / P_.sc) / (P_.bet - P_.delta1);

        S_.y_[State_::V_m] = ((S_.y_[State_::I_input] / P_.sc) / (P_.bet - P_.delta1) - 1);

    } else {
      if (V_.new_Iinput < V_.old_Iinput){
            // To Check if S_.y_[State_::I_adap] is ok; it was Iadap_ini
            S_.y_[State_::I_dep] = S_.y_[State_::I_adap]-(S_.y_[State_::I_input] / P_.sc) / P_.bet;
   ...
   if (lag == from){
	...
	if ( ((double) rand() / (RAND_MAX))			//V_.rng_->drand()
        ...
        if (S_.y_[State_::I_input]<P_.istim_min_spiking_exp){
          ...
          S_.y_[State_::I_adap] = c_aux + (P_.a * exp(P_.b*S_.y_[State_::I_input]) * ((nest::Time::get_resolution().get_ms()-V_.init_sign) * time_scale)) / (P_.alp + (nest::Time::get_resolution().get_ms()-V_.init_sign) * time_scale);
        } else {
          S_.y_[State_::I_adap] = P_.c + (P_.a * exp(P_.b*S_.y_[State_::I_input]) * ((nest::Time::get_resolution().get_ms()-V_.init_sign) * time_scale)) / (P_.alp + (nest::Time::get_resolution().get_ms()-V_.init_sign) * time_scale);
        }
      ...
      if (S_.y_[State_::I_input]<1){
        ...
        S_.y_[State_::I_dep]=P_.Idep_ini_vr;
      }
