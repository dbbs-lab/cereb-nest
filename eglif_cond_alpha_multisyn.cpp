  ...
  for (long lag = from; lag < to; ++lag) {
   ...
   if (lag == from){
	...
	if ( ((double) rand() / (RAND_MAX))			//V_.rng_->drand()
		...
		S_.y_[State_::I_adap] = (P_.a0+P_.a1*P_.a0*(S_.y_[State_::I_input])/1000)*(1 - exp(-(P_.b0*(S_.y_[State_::I_input])/1000)*((nest::Time::get_resolution().get_ms()-V_.init_sign)*P_.tau_m )));
    ...
    if ((nest::Time::get_resolution().get_ms()*P_.tau_m )>P_.ts){
     ...
     //exp_cum((t_final-init_sign)*tao_m,a0+a1*a0*Istim/1000,b0*Istim/1000)*(((t_final*tao_m)>ts)*mul+1)
