...
void eglif_cond_alpha_multisyn::update(nest::Time const &origin,
  ...
  if ( B_.currents_last_value_ < P_.ith){
      ...
      //  std::cout << "Input current lower than ith at time" << V_.time << std::endl;
      ...
      if (V_.new_Iinput < V_.old_Iinput){
          //  std::cout << "Input current decreasing at time" << V_.time << std::endl;
            // To Check if S_.y_[State_::I_adap] is ok; it was Iadap_ini
            S_.y_[State_::I_dep] = curr_conv_fact*(S_.y_[State_::I_adap]-(B_.currents_last_value_ / P_.sc) / P_.bet);
