...
void eglif_cond_alpha_multisyn::update(nest::Time const &origin,
  ...
  if (S_.time < 2){
	 ...
	 // S_.y_[State_::I_adap] = (19.4964*V_.I_gen + 13602)*(1 - exp(-(-0.0002616933875*V_.I_gen + 0.9930255)*S_.time/15.58)) + (-17.3857*V_.I_gen -13402);	//fitting Vittorio ALL Istim
// 21141.0; // fitting Vittorio Istim=1000   //  -898.205; // fitting Vittorio Istim=200 // -694.326;	// fitting Vittorio Istim=600	// -1453.0;	//fitting Annalisa Istim=200		// for hippoE-GLIF
   	 // S_.y_[State_::I_dep] = (-5.9118*V_.I_gen + 8355.5)*(1 - exp(-(-0.0009689269*V_.I_gen + 1.3539033)*S_.time/15.58)) + (6.0689*V_.I_gen -8427.5);	//fitting Vittorio ALL Istim //18620.0; // fitting Vittorio Istim=1000   // 107.936; // fitting Vittorio Istim=200 // 0.275; // fitting Vittorio Istim=600    // 0.03;	//fitting Annalisa Istim=200	// for hippoE-GLIF

   // fitting Vittorio Matematica
 	 //S_.y_[State_::I_adap] = (0.022138719846473493*pow(V_.I_gen,4) - 44.277514352725596*pow(V_.I_gen,3) + 30994.789390682123*pow(V_.I_gen,2) - 8855716.966411779*V_.I_gen + 850150411.3694189)*1000*exp(-(0.0000000394391*pow(V_.I_gen,3)-0.00006471239295063651*pow(V_.I_gen,3) + 0.03427900029366324*V_.I_gen -3.7049552462485305)*S_.time*1000/(-get_C_m()*get_E_L()));
  // S_.y_[State_::I_dep] = (0.022138719846473493*pow(V_.I_gen,4) - 44.277514352725596*pow(V_.I_gen,3) + 30994.789390682123*pow(V_.I_gen,2) - 8855716.966411779*V_.I_gen + 850150411.3694189)*1000*exp(-(0.0000000394391*pow(V_.I_gen,3)-0.00006471239295063651*pow(V_.I_gen,3) + 0.03427900029366324*V_.I_gen -3.7049552462485305)*S_.time*1000/(-get_C_m()*get_E_L()));

// dati Vittorio prova di fitting, ma exp fitting dei dati iniziali da Matlab:
  S_.y_[State_::I_adap] = 0; //79380000*(1-exp(-0.1865*S_.time));
    S_.y_[State_::I_dep] = 0; //79380000*exp(-0.1865*S_.time);
    }

  if ( S_.y_[State_::V_m] < P_.Vmin){
	  S_.y_[State_::V_m] = get_Vmin();
	  //std::cout << " going lower than Vmin simulation!!!!!!!!!!!!! " <<std::endl;
    }
  ...
  for (long lag = from; lag < to; ++lag) {
   ...
   if (lag == from){
	...
	if ( ((double) rand() / (RAND_MAX))			//V_.rng_->drand()
		...
		//S_.y_[State_::I_dep] = 40630*exp(-0.237*S_.time);			// for hippoE-GLIF - fitting Annalisa Istim=200
		//S_.y_[State_::I_dep] = 6943.26*exp(-0.0995*S_.time);			// for hippoE-GLIF - fitting Vittorio Istim=600
               //S_.y_[State_::I_dep] = 3156.03*exp(-0.167*S_.time);			// for hippoE-GLIF - fitting Vittorio Istim=200
               //S_.y_[State_::I_dep] = 1262.0*exp(-0.067*S_.time) // for hippoE-GLIF - fitting Vittorio Istim=1000    //+ 18620.0*(B_.currents_last_value_ - V_.bufferT[P_.bT_size-1])/200.0;
	//	S_.y_[State_::I_dep] =  (-5.9118*V_.I_gen + 8355.5)*(1 - exp(-(-0.0009689269*V_.I_gen + 1.3539033)*S_.time/15.58)) + (6.0689*V_.I_gen -8427.5);	//fitting Vittorio ALL Istim
		//if (S_.y_[State_::I_dep] < 0.0)
		//{S_.y_[State_::I_dep] = 0.0000003;}
		//S_.y_[State_::I_dep] = abs(S_.y_[State_::I_dep]);
	...
	//	S_.y_[State_::I_adap] *= 1;
		...
		S_.y_[State_::I_adap] += P_.A2; 	//+ 21141.0*(B_.currents_last_value_ - V_.bufferT[P_.bT_size-1])/200.0;
//		S_.y_[State_::I_adap] = (19.4964*V_.I_gen + 13602)*(1 - exp(-(-0.0002616933875*V_.I_gen + 0.9930255)*S_.time/15.58)) + (-17.3857*V_.I_gen -13402);	//fitting Vittorio ALL Istim
      //S_.y_[State_::I_adap] = (0.022*pow(V_.I_gen,4) - 44.278*pow(V_.I_gen,3) + 30994.789*pow(V_.I_gen,2) - 8855716.966*V_.I_gen + 850150411.369)*1000*exp(-(0.0000000394391*pow(V_.I_gen,3)-0.00006471239295063651*pow(V_.I_gen,3) + 0.034*V_.I_gen -3.705)*S_.time*1000/(-get_C_m()*get_E_L()));
      //S_.y_[State_::I_dep] = (0.022*pow(V_.I_gen,4) - 44.278*pow(V_.I_gen,3) + 30994.789*pow(V_.I_gen,2) - 8855716.966*V_.I_gen + 850150411.369)*1000*exp(-(0.000*pow(V_.I_gen,3)-0.000*pow(V_.I_gen,3) + 0.034*V_.I_gen -3.705)*S_.time*1000/(-get_C_m()*get_E_L()));
      //std::cout << "I_adap: "<< S_.y_[State_::I_adap] << " I_dep: "<< S_.y_[State_::I_adap] << " partial: " << exp(-(0.000*pow(V_.I_gen,3)-0.000*pow(V_.I_gen,3) + 0.034*V_.I_gen -3.705)*S_.time*1000/(-get_C_m()*get_E_L())) << (0.022*pow(V_.I_gen,4) - 44.278*pow(V_.I_gen,3) + 30994.789*pow(V_.I_gen,2) - 8855716.966*V_.I_gen + 850150411.369)*1000 <<std::endl;

      // dati Vittorio prova di fitting, ma exp fitting dei dati iniziali da Matlab:
    //  S_.y_[State_::I_adap] = 79380000*(1-exp(-0.1865*S_.time));   //38450000*exp(-1.841*S_.time);
    //  S_.y_[State_::I_dep] = 79380000*exp(-0.1865*S_.time);
