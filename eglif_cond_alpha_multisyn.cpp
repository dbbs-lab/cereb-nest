...
void eglif_cond_alpha_multisyn::update(nest::Time const &origin,
  ...
  V_.new_Iinput = B_.currents_last_value_;
  ...
  for (long lag = from; lag < to; ++lag) {
   ...
   if (V_.bufferT[index]>0) {
    ...
    if (V_.r == 0) {
      ...
      if ( B_.currents_last_value_ < P_.ith){
          ...
          if (B_.currents_last_value_ < V_.new_Iinput){
                ...
                S_.y_[State_::I_dep] = curr_conv_fact*(S_.y_[State_::I_adap]/curr_conv_fact + teta * (B_.currents_last_value_ / P_.sc) / P_.bet);

                S_.y_[State_::I_dep] = curr_conv_fact*(S_.y_[State_::I_adap]/curr_conv_fact-(B_.currents_last_value_ / P_.sc) / P_.bet);
      ...
      // std::cout << "Solving ODE with " << S_.y_[State_::V_m] << " " << S_.y_[State_::I_adap] << " " << S_.y_[State_::I_dep] << " input " << B_.currents_last_value_ << " at time " << V_.time << std::endl;
      while (t < step_) {
        const int status =
	...
	if (S_.y_[State_::V_m] > P_.V_th)
        ...
        if (B_.currents_last_value_<P_.istim_min_spiking_exp || B_.currents_last_value_>P_.istim_max_spiking_exp){
          ...
          S_.y_[State_::I_adap] = curr_conv_fact*(P_.c + (P_.a * exp(P_.b*B_.currents_last_value_/1000) * (V_.time -V_.init_sign)) / (P_.alp + (V_.time -V_.init_sign)));
          // std::cout<<"else current range Iadap = "<<S_.y_[State_::I_adap] << " " << curr_conv_fact << " " << time_scale << std::endl;
        }
        ...
        // std::cout<<"spike! Time "<<V_.time<<" Iadap = "<<S_.y_[State_::I_adap] << std::endl;
		/* generated by template org.nest.spl.Statement*/
		//
		/* generated by template org.nest.spl.SmallStatement*/
		/* generated by template org.nest.spl.Assignment*/
