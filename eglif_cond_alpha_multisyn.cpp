...
template <> void RecordablesMap<eglif_cond_alpha_multisyn>::create() {
  ...
  // ignores the Vth with the domain type double
...
eglif_cond_alpha_multisyn_dynamics(double, const double y[], double f[],
  ...
  // Total current
  double I_tot = y[State_::I_dep] - y[State_::I_adap] + node.get_Ie_const() + node.B_.currents_last_value_ + I_syn;
...
// Da eliminare: correnti attive sempre, non solo con stim esterna!! Att: da eliminare anche nel blocco update!
   ...
   //  }
	...
	if ( ((double) rand() / (RAND_MAX))			//V_.rng_->drand()		
		...
		/* generated by template org.nest.spl.SmallStatement*/
		/* generated by template org.nest.spl.Assignment*/
			//S_.y_[State_::I_adap] *= P_.R2;		[Mihalas original model]
