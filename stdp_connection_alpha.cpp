...
STDPAlphaCommonProperties::get_status( DictionaryDatum& d ) const
{
  ...
  if ( vtC_ != 0 )
  {
    def< long >( d, nest::names::volume_transmitter, vtC_->get_node_id() );
  }

  else
  {
    def< long >( d, nest::names::volume_transmitter, -1 );
  }
  ...
  nest::ConnectorModel& cm )
{
  ...
  long vtgid;
  if ( updateValue< long >( d, nest::names::volume_transmitter, vtgid ) )
  {
