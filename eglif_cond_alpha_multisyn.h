...
Empty
*/
class eglif_cond_alpha_multisyn : public Archiving_Node {
public:
  /**
 ...
 /* generated by template org.nest.nestml.function.MemberVariableGetterSetter*/
  ...
  inline RingBuffer &get_spikes_r1() {
    return B_.get_spikes_r1();
  };

  inline RingBuffer &get_spikes_r2() {
    return B_.get_spikes_r2();
  };

  inline RingBuffer &get_spikes_r3() {
    return B_.get_spikes_r3();
  };

  inline RingBuffer &get_currents() { return B_.get_currents(); };
...
protected:
  ...
  friend class RecordablesMap<eglif_cond_alpha_multisyn>;
  friend class UniversalDataLogger<eglif_cond_alpha_multisyn>;
...
private:
  ...
  */
  ...
  struct Buffers_ {
    ...
    inline RingBuffer &get_spikes_r1() { return spikes_r1; }
    RingBuffer spikes_r1;
    //!< Buffer incoming Buffers through delay, as sum
    ;
    double spikes_r1_last_value_;

    inline RingBuffer &get_spikes_r2() { return spikes_r2; }
    RingBuffer spikes_r2;
    //!< Buffer incoming Buffers through delay, as sum
    ;
    double spikes_r2_last_value_;

    inline RingBuffer &get_spikes_r3() { return spikes_r3; }
    RingBuffer spikes_r3;
    //!< Buffer incoming Buffers through delay, as sum
    ;
    double spikes_r3_last_value_;

    inline RingBuffer &get_currents() { return currents; }
    RingBuffer currents;
    //!< Buffer incoming Buffers through delay, as sum
    ;
    ...
    UniversalDataLogger<eglif_cond_alpha_multisyn> logger_;
...
public:
  ...
  */
  ...
  static RecordablesMap<eglif_cond_alpha_multisyn> recordablesMap_;
...
inline port eglif_cond_alpha_multisyn::send_test_event(
    Node &target, rport receptor_type, synindex, bool) {
  // You should usually not change the code in this function.
  // It confirms that the target of connection @c c accepts @c SpikeEvent
  // on
...
inline port
eglif_cond_alpha_multisyn::handles_test_event(SpikeEvent &,
                                                     port receptor_type) {

  if (receptor_type <= 0 ||
      receptor_type > 3	){		//static_cast<port>(get_receptors())) {
...
inline port
eglif_cond_alpha_multisyn::handles_test_event(CurrentEvent &,
                                                     port receptor_type) {
  // You should usually not change the code in this function.
...
inline port eglif_cond_alpha_multisyn::handles_test_event(
...
/* generated by template org.nest.nestml.function.WriteInDictionary*/
  ...
  (*__d)[names::recordables] = recordablesMap_.get_list();
}
